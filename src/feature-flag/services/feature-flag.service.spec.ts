...
describe('FeatureFlagService', () => {
  ...
  describe('isFlagEnabledForUser', () => {
    it('returns false if flag not found', async () => {
      ...
      await expect(service['isFlagEnabledForUser']('flag1', 'user1')).rejects.toThrow('Feature flag not found');
    });
    it('returns false if flag is globally disabled', async () => {
      mockPrisma.featureFlag.findUnique.mockResolvedValue({ enabled: false } as FeatureFlag);
      const result = await service['isFlagEnabledForUser']('flag1', 'user1');
      expect(result).toBe(false);
    });
    it('returns true if rolloutPercentage is 100', async () => {
      mockPrisma.featureFlag.findUnique.mockResolvedValue({ enabled: true, rolloutPercentage: 100 } as FeatureFlag);
      const result = await service['isFlagEnabledForUser']('flag1', 'user1');
      expect(result).toBe(true);
    });
    it('returns false if rolloutPercentage is 0', async () => {
      mockPrisma.featureFlag.findUnique.mockResolvedValue({ enabled: true, rolloutPercentage: 0 } as FeatureFlag);
      const result = await service['isFlagEnabledForUser']('flag1', 'user1');
      expect(result).toBe(false);
    });
  });
